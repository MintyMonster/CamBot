    ...
    public class CommandHandler
    {
        ...
        public async Task CommandExecutedAsync(Optional<CommandInfo> command, ICommandContext context, IResult result) // Add some customisation and logging here
        {
            ...
            if (!command.IsSpecified)
            {
                ...
                await _client.GetGuild(797839905539096637).GetTextChannel(819924744228044800).SendMessageAsync($"**[FAILED]{DateTime.Now.ToString("dd/MM/yy | HH:mm")}** {context.User.Username} Failed to execute a command");
                var cross = new Emoji("\u274C");
            ...
            if (result.IsSuccess)
            {
                ...
                await _client.GetGuild(797839905539096637).GetTextChannel(819924744228044800).SendMessageAsync($"**[SUCCESS] {DateTime.Now.ToString("dd/MM/yy | HH:mm")}** {context.User.Username} Executed a command");
